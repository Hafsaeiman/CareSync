...
function LabReportsPage() {
  ...
  const [previousHash, setPreviousHash] =
    useState("");

  const [medicalChain, setMedicalChain] =
    useState(null);
  ...
  const verifyPatient = async () => {
    ...
    try {
      ...
      const response = await fetch(
        "http://localhost:5000/api/reports/login",
        {
          method: "POST",
      ...
      chain.addBlock(reportBlock);

      setBlockHash(reportBlock.hash);

      setPreviousHash(
        reportBlock.previousHash
      );

      setMedicalChain(chain);
